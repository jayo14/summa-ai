...
6. ✅ Prompt budget, caching, observability — resolved in Milestone 4 (section char cap, TTL cache, error surfacing).
7. Tests — expanded in Milestone 4 (18 new tests, 19 total, all passing). Continue growing alongside new code.
8. Unify auth — depends on Integration Strategy decision (NEXT_STEPS item 8).
9. Cognee production guard — resolved in Milestone 4 (boot-time COGNEE_API_KEY check).
