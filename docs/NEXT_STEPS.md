...
8. ✅ Integration strategy confirmed and Milestone 3 executed — Supabase Auth adopted (backend switch + auth routes proxy). Schema migration script created (`db/migrate_to_supabase.sql`) but needs manual run via Supabase Dashboard SQL Editor (no IPv6 from this server).
