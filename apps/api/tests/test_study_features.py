...
class TestStudyPlanEndpoints:
    ...
    def test_create_study_plan(self):
        ...
        with patch("app.routes.study_routes._store") as mock_store:
            ...
            response = client.post(
                "/api/v1/study-plans", json=payload, headers=_auth_headers()
            )
    ...
    def test_get_study_plan_not_found(self):
        with patch("app.routes.study_routes._store") as mock_store:
            ...
            response = client.get(
                "/api/v1/study-plans/nonexistent", headers=_auth_headers()
            )
    ...
    def test_update_study_plan(self):
        with patch("app.routes.study_routes._store") as mock_store:
            mock_store.update_study_plan = AsyncMock(
                return_value={
                    "id": "p1",
                    "title": "Updated",
                    "progress": 0.8,
    ...
    def test_update_session(self):
        with patch("app.routes.study_routes._store") as mock_store:
            ...
            response = client.patch(
                "/api/v1/study-plans/sessions/s1",
                json={"status": "done"},
                headers=_auth_headers(),
            )
            assert response.status_code == 200
...
class TestFlashcardEndpoints:
    ...
    def test_create_flashcard(self):
        with patch("app.routes.study_routes._store") as mock_store:
            mock_store.create_flashcard = AsyncMock(
                return_value={
                    "id": "f-new",
                    "user_id": "user-1",
                    "front": "Q1",
    ...
    def test_update_flashcard(self):
        with patch("app.routes.study_routes._store") as mock_store:
            mock_store.update_flashcard = AsyncMock(
                return_value={
                    "id": "f1",
                    "mastered": True,
                    "ease_factor": 2.65,
            ...
            response = client.patch(
                "/api/v1/flashcards/f1",
                json={"mastered": True, "interval_days": 2},
                headers=_auth_headers(),
            )
            assert response.status_code == 200
    ...
    def test_delete_flashcard_not_found(self):
        with patch("app.routes.study_routes._store") as mock_store:
            ...
            response = client.delete(
                "/api/v1/flashcards/nonexistent", headers=_auth_headers()
            )
            assert response.status_code == 404
...
class TestExamEndpoints:
    ...
    def test_create_exam(self):
        with patch("app.routes.study_routes._store") as mock_store:
            mock_store.create_exam = AsyncMock(
                return_value={
                    "id": "e-new",
                    "user_id": "user-1",
                    "name": "Calculus",
            ...
            response = client.post(
                "/api/v1/exams",
                json={"name": "Calculus", "exam_date": "2026-08-15"},
                headers=_auth_headers(),
            )
            assert response.status_code == 201
    ...
    def test_update_exam(self):
        with patch("app.routes.study_routes._store") as mock_store:
            mock_store.update_exam = AsyncMock(
                return_value={
                    "id": "e1",
                    "name": "Calculus II",
                    "exam_date": "2026-08-15",
            ...
            response = client.patch(
                "/api/v1/exams/e1", json={"readiness": 75}, headers=_auth_headers()
            )
            assert response.status_code == 200
    ...
    def test_delete_exam_not_found(self):
        with patch("app.routes.study_routes._store") as mock_store:
            ...
            response = client.delete(
                "/api/v1/exams/nonexistent", headers=_auth_headers()
            )
            assert response.status_code == 404
