...
async def test_list_artifacts_empty():
    ...
    assert await store.list_artifacts("user-1") == []
...
async def test_get_artifact_found():
    ...
    assert await store.get_artifact("a1") == {"id": "a1"}
...
async def test_update_artifact():
    ...
    conn.fetchrow.side_effect = [
        {"id": "a1", "title": "Updated", "current_version": 2, "component": None},
        {"id": "a1", "title": "Updated", "current_version": 2, "component": None},
    ]
    result = await store.update_artifact("a1", {"title": "Updated"})
...
async def test_delete_artifact_success():
    ...
    assert await store.delete_artifact("a1") is True


@pytest.mark.asyncio
async def test_delete_artifact_missing():
    store = DataStore()
    pool, conn = _mock_pool(store)
...
@pytest.mark.asyncio
async def test_toggle_pin_artifact():
    store = DataStore()
    pool, conn = _mock_pool(store)
    conn.fetchrow.return_value = {"id": "a1", "pinned": True}
    result = await store.toggle_pin_artifact("a1")
...
async def test_create_conversation():
    ...
    result = await store.create_conversation("user-1", title="New chat")
    assert result["id"] == "c1"
...
async def test_get_conversation():
    ...
    assert await store.get_conversation("c1") == {"id": "c1"}
...
async def test_list_messages():
    ...
    result = await store.list_messages("c1")
...
async def test_add_message():
    ...
    result = await store.add_message("c1", "user", "Hello")
    assert result["id"] == "m1"
...
async def test_list_timeline_events():
    ...
    result = await store.list_timeline_events("user-1")
...
async def test_create_timeline_event():
    ...
    result = await store.create_timeline_event("user-1", "milestone", "Title", "Desc")
    assert result["id"] == "t1"
...
async def test_list_materials():
    ...
    result = await store.list_materials("user-1")
...
async def test_create_material():
    ...
    result = await store.create_material("user-1", {"type": "pdf", "title": "Notes"})
...
async def test_delete_material():
    ...
    pool, conn = _mock_pool(store)
    conn.execute.return_value = "DELETE 1"
    assert await store.delete_material("mat1") is True
...
async def test_list_concepts():
    ...
    result = await store.list_concepts("user-1")
...
async def test_create_concept():
    ...
    result = await store.create_concept("user-1", {"name": "Polymorphism"})
...
async def test_delete_concept():
    ...
    pool, conn = _mock_pool(store)
    conn.execute.return_value = "DELETE 1"
    assert await store.delete_concept("con1") is True


@pytest.mark.asyncio
async def test_list_artifact_versions():
    store = DataStore()
    pool, conn = _mock_pool(store)
    conn.fetch.return_value = [{"id": "v1", "version": 1}]
    result = await store.list_artifact_versions("a1")
    assert len(result) == 1


@pytest.mark.asyncio
async def test_restore_artifact_version_found():
    store = DataStore()
    pool, conn = _mock_pool(store)
    conn.fetchrow.side_effect = [
    ...
    result = await store.restore_artifact_version("a1", 1)
    assert result["title"] == "Old"


@pytest.mark.asyncio
async def test_restore_artifact_version_missing():
    store = DataStore()
    pool, conn = _mock_pool(store)
    conn.fetchrow.return_value = None
    assert await store.restore_artifact_version("a1", 99) is None
