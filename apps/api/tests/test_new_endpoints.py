...
class TestWebSocketBroadcast:
    def test_broadcast_disabled(self):
        with patch("app.main.verify_supabase_jwt", return_value="user-1"), \
             patch("app.config.settings.WEBSOCKET_ENABLED", False):
    ...
    def test_broadcast_enabled(self):
        with patch("app.main.manager") as mock_manager, \
             patch("app.main.verify_supabase_jwt", return_value="user-1"):
            mock_manager.broadcast = AsyncMock()
            response = client.post("/ws/broadcast", json={"type": "announcement", "text": "hello"})
...
class TestAnalyticsCacheHeaders:
    def test_analytics_cache_control(self):
        with patch("app.routes.data_routes.resolve_user_id", return_value="test-user"), \
