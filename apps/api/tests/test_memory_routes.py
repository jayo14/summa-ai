...
async def test_remember_text():
    with patch("app.routes.memory.cognee") as mock_cognee:
        ...
        try:
            response = client.post("/api/v1/memory/remember/text", headers=_auth_headers(), json={
                "text": "I like Python",
...
async def test_remember_conversation():
    with patch("app.routes.memory.cognee") as mock_cognee:
        ...
        token = set_current_user_id("user-1")
        try:
            response = client.post("/api/v1/memory/remember/conversation", headers=_auth_headers(), json={
...
async def test_hybrid_extract():
    with patch("app.routes.memory.summastudy_memory") as mock_memory:
        mock_memory.extract_and_store_memories = AsyncMock(return_value=[
            {"content": "likes Python", "type": "preference"}
        ])
        token = set_current_user_id("user-1")
        try:
            response = client.post("/api/v1/memory/hybrid/extract", headers=_auth_headers(), json={
                "message": "I really enjoy coding in Python"
            })
            assert response.status_code == 200
            data = response.json()
            assert data["count"] == 1
...
async def test_hybrid_facts():
    with patch("app.routes.memory.summastudy_memory") as mock_memory:
        mock_memory.retrieve_relevant_memories = AsyncMock(return_value=[
            {"content": "goal: learn AI", "type": "goal"}
        ])
        token = set_current_user_id("user-1")
        try:
            response = client.get("/api/v1/memory/hybrid/facts?limit=10", headers=_auth_headers())
            assert response.status_code == 200
            data = response.json()
...
async def test_hybrid_summary():
    with patch("app.routes.memory.summastudy_memory") as mock_memory:
        mock_memory.get_memory_summary = AsyncMock(return_value={"total": 2, "by_type": {"preference": 2}})
        token = set_current_user_id("user-1")
        try:
            response = client.get("/api/v1/memory/hybrid/summary", headers=_auth_headers())
            assert response.status_code == 200
            data = response.json()
            assert data["total"] == 2
        finally:
...
async def test_hybrid_context():
    with patch("app.routes.memory.cognee") as mock_cognee, \
         patch("app.routes.memory.summastudy_memory") as mock_memory:
        ...
        token = set_current_user_id("user-1")
        try:
            response = client.get("/api/v1/memory/hybrid/context", headers=_auth_headers())
            assert response.status_code == 200
            data = response.json()
...
async def test_memory_forget():
    with patch("app.routes.memory.cognee") as mock_cognee:
        ...
        token = set_current_user_id("user-1")
        try:
            response = client.post("/api/v1/memory/forget", headers=_auth_headers(), json={
                "topic": "loops"
            })
            assert response.status_code == 200
        finally:
...
async def test_memory_feedback():
    with patch("app.routes.memory.cognee") as mock_cognee:
        ...
        token = set_current_user_id("user-1")
        try:
            response = client.post("/api/v1/memory/feedback", headers=_auth_headers(), json={
                "session_id": "s1",
                "score": 5,
                "text": "Great session",
            })
...
async def test_memory_consolidate():
    with patch("app.routes.memory.cognee") as mock_cognee:
        ...
        token = set_current_user_id("user-1")
        try:
            response = client.post("/api/v1/memory/consolidate", headers=_auth_headers())
            assert response.status_code == 200
            data = response.json()
