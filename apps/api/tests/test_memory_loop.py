...
async def test_memory_loop_integration():
    ...
    mock_cognee_api.recall.return_value = [{"text": "Previous context: User mentioned they like NLP.", "metadata": {}}]
    mock_cognee_api.remember = AsyncMock()

    # Inject the mock into the cognee service instance
    cognee._cognee = mock_cognee_api

    with patch("app.routes.chat.resolve_user_id", return_value="test_user"),          patch("app.routes.chat._stream_zai") as mock_stream:
